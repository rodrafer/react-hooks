...
The `useState` is a hook meant to manage state variables in React components. With such variables your app can react to user interactions and update the view.

The hook takes the initial state as an argument, which can be of any type, and returns an array of two elements: the state variable and a function to update that state.

On the first render of the component, the state variable asumes the value of the initial state passed to `useState` hook. The function to update its state is usually called inside event handlers, and it takes the next state as an argument (or a function called "updater function").

After calling this function, React will trigger a re-render of the component with the updated state of the variable. However, this update is queued by React, so if you try to access the state variable before the next render is triggered, you will get an outdated value of the variable. 
