...
On the first render of the component, the state variable asumes the value of the initial state passed to useState hook. The function to update its state is usually called inside event handlers, and it takes the next state as an argument.

After calling this function, React will trigger a re-render of the component with the updated state of the variable. However, this update is queued by React, so if you try to access the state variable before the next render is triggered, you will get outdated value of the variable.
